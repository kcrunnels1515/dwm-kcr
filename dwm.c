...
enum { SchemeNorm, SchemeSel, SchemeInv, SchemeNet, SchemeStat, SchemeDate }; /* color schemes */
...
static int horseshit(Client *c, Atom proto);
...
static const char broken[] = "dont work :(";
...
killunsel(const Arg *arg)
{
	...
	for (i = selmon->clients; i; i = i->next) {
		if (ISVISIBLE(i) && i != selmon->sel) {
			if (!horseshit(i, wmatom[WMDelete])) {
				XGrabServer(dpy);
				XSetErrorHandler(xerrordummy);
				XSetCloseDownMode(dpy, DestroyAll);
